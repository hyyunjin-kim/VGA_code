...
module FreezeMux (
    ...
    always_comb begin
        if (y_pixel == scan_y)
            {r_out, g_out, b_out} = 12'h00F;  // scan line 표시
        else if (y_pixel < scan_y)
            {r_out, g_out, b_out} = {r_freeze, g_freeze, b_freeze};  // freeze buffer
        else
            {r_out, g_out, b_out} = {cam_r, cam_g, cam_b};  // frame buffer
    end
